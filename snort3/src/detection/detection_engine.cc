...
// snortsharp integration
#include "snortsharp/snort3_snortsharp_bridge.hpp"
...
void DetectionEngine::thread_term()
{
    ...
    // shutdown snortsharp parallel processing
...
void DetectionEngine::init()
{
    ...
    // initialize snortsharp parallel processing
    if(!SnortSharpBridge::initialize()) {
        // snortsharp initialization failed, but continue with snort3 only
        printf("[Snort3] Warning: SnortSharp integration failed to initialize\n");
    }
}
...
bool DetectionEngine::inspect(Packet* p)
{
    bool inspected = false;
    {
        ...
        else
        {
            ...
            // snortsharp parallel processing integration
            // send packet to parallel flow analysis after snort3 detection
