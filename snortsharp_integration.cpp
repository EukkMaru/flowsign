...
#include "snort3/src/protocols/packet.h"
#include "snort3/src/protocols/tcp.h"
#include "snort3/src/framework/decode_data.h"
#include "snort3/src/protocols/ip.h"
...
std::unique_ptr<SnortPacket> convert_snort_packet_from_raw(const void* snort_internal_packet) {
    ...
    if(snort_packet->ptrs.ip_api.is_ip4()) {
        ...
        if(src_ip && dst_ip) {
            our_packet->src_ip = src_ip->get_ip4_value(); // Copy value, not reference
            our_packet->dst_ip = dst_ip->get_ip4_value(); // Copy value, not reference
        }
